...
def baseline( df ):
    c = \
        0.5 * (df["mm_lluvia_dia"].isna()) * 0.5 + \
        0.5 * (df["llovieron_hamburguesas_hoy"]=="si") * 0.5 + \
        0.3 * (df["horas_de_sol"]<7) * 0.3 + \
        0.3 * (df["humedad_tarde"]>60) * 0.3 + \
        0.3 *(df["nubosidad_tarde"]>7) * 0.3 + \
        0.1*(df["rafaga_viento_max_direccion"].isin(['Noroeste', 'Oestenoroeste', 'Norte', 'Oeste', 'Nornoreste', 'Oestesuroeste' ])) + \
        0.1*(df["barrio"].isin(['Parque Patricios', 'Villa Pueyrredón', 'Saavedra', 'Chacarita', 'Recoleta', 'Vélez Sársfield', 'Barracas', 'Villa del Parque', 'Villa Devoto', 'Caballito', 'Monte Castro', 'La Boca', 'Villa Soldati', 'San Cristóbal', 'Monserrat', 'Flores', 'Parque Avellaneda', 'Constitución', 'Puerto Madero', 'Boedo', 'Villa Real', 'La Paternal', 'Villa Riachuelo']))
